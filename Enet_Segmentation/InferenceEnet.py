...
if(scriptParameter.dataset == dataset.riverblindness):
    ...
    trained_model = torch.load("./modelBackups/RiverBlindnessEvaluation_15_02_23_best/ckpt-enet-30-4.709150105714798.pth",  map_location=device)
elif(scriptParameter.dataset == dataset.schistosoma):
    print("[INFO] Using schistosoma model.")
    trained_model = torch.load("./modelBackups/SchistosomaMansoni_12_02_2023_best/ckpt-enet-120-2.7430253960192204.pth",  map_location=device)
...
if(scriptParameter.vo == VisualOption.contour):
    ...
    for iy, ix in np.ndindex(input_img_copy_to_save.shape[:2]):
        input_img_copy_to_save[iy, ix, 3] = 0
